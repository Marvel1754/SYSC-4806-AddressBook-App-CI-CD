...
public class AddressBookControllerWebLayerTest {
    @Autowired
    ...
    public void testAddressBooksCommandWorks() throws Exception {
        when(controller.addressBooks()).thenReturn(new ArrayList<AddressBook>());
        endpointCommand = "/addressbooks";
        this.mockMvc.perform(get(endpointCommand)).andDo(print()).andExpect(status().isOk());
    }

    @Test
    public void testCreateCommandWorks() throws Exception {
        when(controller.create(any())).thenReturn(new AddressBook());
